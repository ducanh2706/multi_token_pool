module pool_addr::Multi_Token_Pool {
    ...
    public entry fun join_pool(sender: &signer, pool_amount_out: u64, max_amounts_in: vector<u64>) acquires TokenList, TokenRecord, ManagedFungibleAsset {
        let sender_addr = signer::address_of(sender);
        let pool_total = get_total_supply(sender);
    ...
    public fun exit_pool(sender: &signer, pool_amount_in: u64, min_amounts_out: vector<u64>) acquires TokenList, TokenRecord, ManagedFungibleAsset {
        let sender_addr = signer::address_of(sender);
        let pool_total = get_total_supply(sender);
    ...
    fun get_total_supply(sender: &signer): u64 {
        let sender_addr = signer::address_of(sender);
